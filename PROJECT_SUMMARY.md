...
- **多模态支持**:
  ...
  - 多图预览
- **对话导出**:
  - **[New]** 支持将当前对话导出为 Markdown 文件，包含时间戳和图片链接。

### 3. 现代化 UI/UX (Complete)
- **视觉设计**: 采用紫色渐变背景，配合半透明毛玻璃效果，界面现代美观。
- **设置面板**: 完整的设置界面，支持调整 Temperature, Max Tokens, 流式开关等。
- **响应式布局**: 完美适配桌面端和移动端（包含针对 iOS Safari 的安全区域适配）。
- **暗色模式**: 默认深色主题，保护视力。
- **会话管理**: 
  - 支持新建、切换、删除会话。
  - 支持重命名会话（双击或点击编辑图标）。
- **交互反馈**:
  - 全局 Toast 通知 (react-hot-toast)，优雅展示错误和成功信息。
  - 错误边界 (Error Boundaries)，防止渲染错误导致应用崩溃。
